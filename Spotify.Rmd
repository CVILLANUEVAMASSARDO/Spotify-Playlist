...
```{r}
get_clust_tendency(beats_escalar, n = 40, graph = FALSE)
```
...
```{r, warning=FALSE}
datos <- data.frame(beats_escalar)

# Calcular SSE para diferentes valores de K
k_values <- 1:30
sse_values <- c()
for (k in k_values) {
  kmeans_model <- kmeans(datos, centers = k)
  sse_values <- c(sse_values, kmeans_model$tot.withinss)
}
...
# uso distancia euclidiana
tempDist <- dist(beats_escalar) %>% as.matrix()

#reordeno filas y columnas en base al cluster obtenido
index <- sort(beats_escalar_kmeans$cluster, index.return=TRUE)
tempDist <- tempDist[index$ix,index$ix]
rownames(tempDist) <- c(1:nrow(escal_data_pok))
colnames(tempDist) <- c(1:nrow(escal_data_pok))
...
```{r}
escal_data_pok <- data.frame(beats_escalar)
head(escal_data_pok)
```
